...
# env = wrap_env(gym.make('CarRacing-v0', verbose = False))
...
if load_checkpoint:
    ...
    #Load last models    
    global_counter = 1197338 #Set by the id of model that want to load
    print(global_counter)            
    saver.restore(sess, f"model/m.ckpt-{global_counter}")    
    dqn_agent.global_counter = global_counter
    # dqn_agent.episode_counter = 0
    if(is_countinue_train):
        dqn_agent.do_training = True
        render = False
        load_checkpoint = False #Continute training    
    else:
        render = True
...
def main_loop(eps_history,dqn_scores,avg_score_all,render,load_checkpoint):
    ...
    while True:
        if list:
            break
        if dqn_agent.episode_counter >= train_episodes:
